    ...
    original_base.class_eval do
      class << self
        ...
        def __record_method_call(method_name, *args, &block)
          SuperModule.log "#{self.inspect} requests recording method call: #{__method_signature(method_name, args)}"
          return if self.is_a?(Class)
          SuperModule.log "#{self.inspect} recording request for method call: #{__method_signature(method_name, args)} has been accepted"
        ...
        def __invoke_module_body_method_calls(base)
          ...
          __all_module_body_method_calls_in_definition_order.each do |method_name, args, block|
            SuperModule.log "Invoking #{base.inspect}.#{__method_signature(method_name, args)}"
